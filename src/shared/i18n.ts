...
export const zhTranslations: Record<string, string> = {
  ...
  Shortcuts: '快捷键',
  'Click a shortcut to rebind it. Backspace removes it; Esc cancels.':
    '点击键位改绑。Backspace 删除；Esc 取消。',
  ...
  'Toggle minimize to tray': '切换最小化到托盘',
  'Works while the app is in the tray': '应用在托盘时也能切换',
  ...
  'Search and capture durable memories across coding sessions.': '跨编码会话检索与记录持久记忆。',
  Save: '保存',
  // Skills & MCP
  Skills: '技能',
  Skill: '技能',
  ...
  'Trust this host and add its key to known_hosts?': '信任此主机并把密钥写入 known_hosts？',
  Trust: '信任',
  'Could not save host key.': '无法保存主机密钥。',
  ...
  'Local branches': '本地分支',
  'Search local branches': '搜索本地分支',
  'Switching branches affects all conversations in this workspace.':
    '切换分支会影响此工作区的所有会话。',
  'Create branch from HEAD…': '从当前 HEAD 新建分支…',
  'Create branch': '新建分支',
  'Create and switch': '新建并切换',
  'Branch name': '分支名称',
  'Detached HEAD': '分离 HEAD',
  'No matching local branches.': '没有匹配的本地分支。',
  'Failed to load branches': '读取分支失败',
  ...
  'Creates a branch from the current HEAD and switches every conversation in this workspace to it.':
    '从当前 HEAD 新建分支，并将此工作区的所有会话切换到该分支。',
  'Create a commit before creating a branch from HEAD.': '请先创建一个提交，再从 HEAD 新建分支。',
  'Stop running conversations and coworkers in this workspace before switching branches.':
    '请先停止此工作区中运行的会话和协作者，再切换分支。',
  'Commit or stash uncommitted changes before switching branches.':
    '请先提交或暂存（stash）未提交改动，再切换分支。',
  'This workspace is busy. Try again when its conversations are ready.':
    '此工作区正忙，请等待会话就绪后重试。',
  'Branch is checked out in another worktree.': '此分支已被其他 worktree 占用。',
  'Enter a valid Git branch name.': '请输入有效的 Git 分支名称。',
  'A branch with this name already exists.': '同名分支已存在。',
  'This branch no longer exists. Refresh the branch list.': '此分支已不存在，请刷新分支列表。',
  'This workspace is already on that branch.': '此工作区已在该分支上。',
  'Branches are not available for this workspace.': '此工作区无法使用 Git 分支。',
  'Git could not switch branches.': 'Git 无法切换分支。',
  'Wait for the workspace operation to finish before saving.': '请等待工作区操作完成后再保存文件。',
  ...
  'Used for semantic search and near-duplicate detection. Without it search still works using full-text only.':
    '用于语义检索与近重复识别。没有它检索仍可用,只是退化为纯全文。',
  'Remote (OpenAI-compatible)': '远程（OpenAI 兼容）',
  'Unknown model; the default is used instead.': '未知模型,实际使用默认模型。',
  ...
  Distillation: '提炼',
  'Distillation model': '提炼模型',
  'Reads finished sessions and writes the memories.': '读已结束的会话并写出记忆。',
  'Follows the title-summary model': '跟随标题模型',
  'Memory language': '记忆语言',
  ...
  Backfill: '补齐',
  'Backfilling…': '补齐中…',
  'Vectors unavailable: {{reason}}': '无法生成向量：{{reason}}',
  'Database size': '库体积',
  Spaces: '空间数',
  'Distill past sessions': '提炼历史会话',
  'The automatic switch only covers sessions that end from now on. Distill earlier sessions here to fill the library.':
    '自动开关只对从现在开始结束或被回收的会话生效。早于此的会话在这里手动提炼，否则记忆库一直是空的。',
  'No recorded sessions found.': '没有找到已记录的会话。',
  'No project': '无项目',
  'Already distilled': '已提炼',
  'Distill all ({{count}})': '全部提炼（{{count}}）',
  'All sessions': '全部会话',
  'View sessions': '会话',
  'Some sessions were already distilled': '部分会话已经提炼过',
  '{{label}}: {{total}} sessions, {{done}} already distilled. Re-distilling runs the model again and may add new memories alongside the existing ones.':
    '{{label}}：共 {{total}} 个会话，其中 {{done}} 个已提炼。重新提炼会再跑一次模型，可能在现有记忆之外新增几条。',
  'Only the {{count}} remaining': '只提炼剩下的 {{count}} 个',
  'Re-distill all {{count}}': '重新提炼全部 {{count}} 个',
  'Distill project': '提炼本项目',
  '{{total}} sessions · {{pending}} not distilled': '{{total}} 个会话 · {{pending}} 个未提炼',
  Distill: '提炼',
  'Distill again': '重新提炼',
  'Distilling…': '提炼中…',
  'Background tasks': '后台任务',
  'Clear finished tasks': '清空历史任务',
  'Removes completed and cancelled jobs. Running tasks are kept. Sessions will show as not distilled again, since that flag is derived from job history — re-distilling is deduplicated, so it will not create duplicate memories.':
    '清除已完成和已作废的任务，进行中的保留。清除后会话会重新显示为「未提炼」——该标记是由任务历史推导的；重新提炼有去重，不会产生重复记忆。',
  'Removes completed and cancelled jobs. Running tasks are kept.':
    '只删除已完成和已作废的记录，不影响进行中的任务。',
  'Re-embedding memories': '正在重建向量',
  'Nothing running.': '没有正在运行的任务。',
  'Turn memory off in Built-in tools.': '如需关闭，请到「内置工具」中操作。',
  critical: '关键',
  important: '重要',
  useful: '有用',
  low: '次要',
  'Importance {{value}}': '重要度 {{value}}',
  '(legacy)': '（旧版）',
  'superseded by the GGUF build': '已被 GGUF 版取代',
  'job:distill': '提炼',
  'job:entities': '实体',
  'job:vectors': '向量',
  'Show less': '收起',
  'Show all {{count}}': '查看全部 {{count}} 条',
  ...
  'Browse what the agent knows as a graph or a tree, ask for an interpretation, and consolidate related memories into a crystal.':
    '用图谱或树浏览 agent 已知的知识，让它解读，并把相关记忆合成结晶。',
  Graph: '图谱',
  Tree: '知识树',
  'By type': '按类型',
  'By entity': '按实体',
  ...
  'No entities yet. Turn on entity extraction and store a few memories first.':
    '还没有实体。先打开「抽取实体」并存下几条记忆。',
  'Colour = entity type · size = how many memories mention it':
    '颜色 = 实体类型 · 大小 = 被多少条记忆提及',
  '{{shown}} of {{total}} entities': '{{total}} 个实体中的 {{shown}} 个',
  Fit: '适应窗口',
  'Fit to view': '缩放到全图',
  Relayout: '重新布局',
  'Re-run layout': '重新计算布局',
  '{{count}} memories · {{space}}': '{{count}} 条记忆 · {{space}}',
  Interpret: '解读',
  'Interpreting…': '解读中…',
  Interpretation: '解读',
  'Based on {{count}} memories': '基于 {{count}} 条记忆',
  'Crystallize ({{min}}+)': '结晶（{{min}} 条起）',
  'Synthesizing…': '合成中…',
  ...
  'Everything the agent has saved. Archiving hides a memory from search but keeps it.':
    'agent 存下的全部记忆。归档只是让它不再出现在检索里，内容仍保留。',
  'Clear selection': '取消选择',
  'Archive selected': '归档所选',
  'Select memory': '选中记忆',
  'Delete {{count}} memories permanently?': '永久删除这 {{count}} 条记忆?',
  'Search memories': '搜索记忆',
  'All spaces': '全部空间',
  'All types': '全部类型',
  'Show archived': '显示已归档',
  'No memories yet. They appear here once the agent stores something.':
    '还没有记忆。agent 存下第一条后会出现在这里。',
  'No memories match these filters.': '没有符合当前筛选条件的记忆。',
  'Type a query to search by meaning.': '输入关键词后按语义检索。',
  Crystal: '结晶',
  '{{count}} hits': '命中 {{count}} 次',
  Previous: '上一页',
  'Needs review': '待审阅',
  'Reviewing only records your judgement. Nothing is deleted and no memory is superseded.':
    '审阅只记录你的判断,不会删除任何记忆,也不会让任何一条被取代。',
  Accept: '接受',
  Dismiss: '忽略',
  Type: '类型',
  Space: '空间',
  ...
  'Event date': '事件日期',
  Embedding: '向量模型',
  'Synthesized from': '合成自',
  'Related versions': '相关版本',
  'Delete permanently': '永久删除',
  'Delete this memory permanently?': '永久删除这条记忆?',
  ...
  'Applies to newly started conversations.': '对新启动的会话生效。',
  'Follow parent': '跟随父会话',
  'Depth: {{level}}': '深度：{{level}}',
  Disabled: '已停用',
  'Select model': '选择模型',
  Toolset: '工具集',
  ...
  'Earlier activity details were cleared; the final report is still available.':
    '较早的过程详情已清理，最终报告仍然保留。',
  '(no output yet)': '(暂无输出)',
  Deny: '拒绝',
  Allow: '允许',
  ...
  'Rewinding conversation and files…': '正在回退对话并还原文件…',
