...
describe('applyGuestEvent（单条事件）', () => {
  it('message-upsert 按 index 幂等写入，并返回 lastIndex 供调用方存游标', () => {
    const r = applyGuestEvent(emptyGuestView(), { type: 'message-upsert', index: 3, message: msg('a') });
  ...
  it('status / turn-completed / turn-failed 流转，且清除 retry', () => {
    const base = { ...emptyGuestView(), retry: { attempt: 1, maxAttempts: 3, delayMs: 1, error: 'x', at: 0 } };
    expect(applyGuestEvent(base, { type: 'status', status: 'running' }).view.status).toBe('running');
  ...
  it('turn-retry 设置 retry 横幅（now 可注入）', () => {
    ...
    expect(r.view.retry).toEqual({ attempt: 2, maxAttempts: 5, delayMs: 800, error: 'rate limited', at: 42 });
  });
  ...
  it('approval-request 兼容嵌套 request 与平铺两种形状；approval-resolved 按 requestId 删除', () => {
    ...
    const flat = applyGuestEvent(nested.view, { type: 'approval-request', requestId: 'r2', toolName: 'edit' });
  ...
  it('ask-request 兼容嵌套 ask 与平铺；ask-resolved 删除', () => {
    const a = applyGuestEvent(emptyGuestView(), { type: 'ask-request', ask: { requestId: 'q1', question: '?' } });
    const b = applyGuestEvent(a.view, { type: 'ask-request', requestId: 'q2', question: '??' });
    expect(b.view.asks.map((x) => x.requestId)).toEqual(['q1', 'q2']);
    expect(
      applyGuestEvent(b.view, { type: 'ask-resolved', requestId: 'q2' }).view.asks.map((x) => x.requestId)
    ).toEqual(['q1']);
  });
  ...
  it('task-started / subagent-update 走任务投影，不动消息', () => {
    ...
    const s = applyGuestEvent(r.view, {
      ...
      agent: { id: 'a1', description: 'd', status: 'running', steps: 0, currentActivity: '', startedAt: 1 },
    });
...
describe('applyGuestSnapshot（尾窗快照）', () => {
  ...
  it('尾窗与已有内容接得上（base <= prevMax+1）→ 合并保留上滑加载的早段', () => {
    const existing = new Map([['s1', viewWith([[0, 'old0'], [1, 'old1']])]]);
  ...
  it('空消息快照 lastIndex 为 undefined', () => {
    const out = applyGuestSnapshot(new Map(), { sessions: [snap('s1', 0, [])] });
    expect(out[0].lastIndex).toBeUndefined();
  });
});
