...
/** 通道顺序与 searchMemories 一致：FTS / vector / entity / community */
export function channelWeights(intent: SearchIntent): [number, number, number, number] {
  if (intent === 'relationship') return [0.8, 1, 1.3, 1.1];
  if (intent === 'conceptual') return [0.8, 1.2, 0.8, 1];
  return [1.2, 1, 0.8, 0.4];
}
