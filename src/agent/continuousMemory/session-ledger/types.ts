...
/** 压实合成的记忆边界观察 id；不写 ledger、不可 recall。 */
export const CARRIED_BOUNDARY_ID = '000000000000';
...
export function isMemoryDetails(value: unknown): value is MemoryDetails {
  ...
  return (
    ...
    Array.isArray(value.reflections) &&
    value.reflections.every(isReflection) &&
    (value.coveredThroughEntryId === undefined || isNonEmptyString(value.coveredThroughEntryId)) &&
    (value.priorSummary === undefined || isNonEmptyString(value.priorSummary))
  );
}
