...
- [ ] 写临时 node 脚本验证：sdk 在项目 ESM 环境可 import；StdioClientTransport 连
      `npx -y @modelcontextprotocol/server-everything`（官方测试 server）能 listTools
- 验证：脚本打印工具列表；验后删脚本

### 2. 协议与 main 侧下发
- [ ] `src/shared/types/agent.ts`：spawn 命令加 `mcpServers?: McpServerSpawnConfig[]`
      （name/transport/command/args/env/url，全部可选字段按 transport 校验从宽）
- [ ] `src/main/services/agentHost.ts`：`enabledMcpServers()` 从 settings 取
...
- [ ] `src/agent/mcp.ts`：
  ...
  - `closeAll()`：断开全部连接
- [ ] `src/agent/supervisor.ts`：spawn 里 `customTools: await mcpManager.toolsFor(mcpServers)`；
      worker 退出路径调 closeAll
- 验证：`pnpm typecheck && pnpm lint && pnpm test`

### 4. 真机验证（CDP，参考 docs/dev-debugging.md）
- [ ] 设置里加一条 stdio server（server-everything 或 filesystem）
- [ ] 重启 `pnpm dev`（worker 不热重载）
- [ ] 新会话发消息让模型调用 MCP 工具（如 everything 的 echo），确认调用成功、结果显示
- [ ] 禁用该 server → 新会话无其工具
- [ ] 配置坏 command 的 server → spawn 正常、对话可用、console 有错误留痕
- [ ] 退出 app → `ps` 无残留 MCP 子进程
- [ ] 清理测试 server 条目与测试对话

### 5. 收尾
- [ ] 全量 `pnpm typecheck && pnpm lint && pnpm test`
- [ ] 小步提交（协议+main 下发 / worker McpManager / 修正项 分开）
- [ ] 若实现期学到可沉淀的契约（sdk 在 utilityProcess 的坑等），更新 spec
